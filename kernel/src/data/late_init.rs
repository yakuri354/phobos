...
impl<T> LateInit<T> {
    ...
    pub unsafe fn init(&self, data: T) -> bool {
        ...
        if mut_ref.init.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_err() {
            false
        } else {
...
impl<T> Deref for LateInit<T> {
    ...
    fn deref(&self) -> &Self::Target {
        assert!(self.init.load(Ordering::SeqCst), "LateInit dereferenced but uninitialized");
